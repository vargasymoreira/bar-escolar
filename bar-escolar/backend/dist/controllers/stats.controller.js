...
class StatsController {
        ...
        try {
            ...
            pedidos.forEach(pedido => {
                ...
                if (salesByDay[dateStr]) {
                    ...
                    const total = pedido.items.reduce((sum, item) => sum + (Number(item.producto.precio) * item.cantidad), 0);
    ...
    static async getBestSelling(req, res) {
        try {
            ...
            const items = await prisma_service_1.default.pedidoItem.groupBy({
                by: ['productoId'],
                _sum: {
                    cantidad: true
                },
    ...
    static async getRevenue(req, res) {
        try {
            ...
            const totalRevenue = pedidos.reduce((sum, pedido) => {
                const pedidoTotal = pedido.items.reduce((itemSum, item) => itemSum + (Number(item.producto.precio) * item.cantidad), 0);
                return sum + pedidoTotal;
            }, 0);
            const totalOrders = pedidos.length;
            const totalItems = pedidos.reduce((sum, pedido) => sum + pedido.items.reduce((itemSum, item) => itemSum + item.cantidad, 0), 0);
    ...
    static async getSalesByCategory(req, res) {
        try {
            const items = await prisma_service_1.default.pedidoItem.findMany({
                include: {
                    producto: true
                }
            });
    ...
    static async getDashboardSummary(req, res) {
        try {
            ...
            const [totalPedidos, pedidosHoy, totalProductos, totalIngresos] = await Promise.all([
                prisma_service_1.default.pedido.count(),
                prisma_service_1.default.pedido.count({
                    where: {
                        fechaPedido: {
                            gte: today
                ...
                prisma_service_1.default.producto.count({
                    where: {
                        disponible: true
                    }
                }),
                prisma_service_1.default.pedido.findMany({
                    include: {
                        items: {
                            include: {
                                producto: true
